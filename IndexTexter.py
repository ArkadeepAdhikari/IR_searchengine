...
def extractText(content, docId, ngrams, score_dict, anchor_text_file):
    ...
    finalText = ' '.join([seg.text for seg in bold_segments])  
    ...
    for seg in soup.find_all('a'):
        anchor_text = processText(seg.text)
        if anchor_text == '' or anchor_text == 'here': continue
    ...
    for folder in subfolders:
        for file in os.listdir(folder):
            ...
            with open(folder + '/' + file) as f:
                ...
                if url != 'https://www.ics.uci.edu/~eppstein/junkyard/all.html': continue
